...
For more information, see the [Python SDK](/docs/en/python-sdk/sync/git), [TypeScript SDK](/docs/en/typescript-sdk/git) and [API](/docs/en/tools/api/#daytona-toolbox) references:
...
Daytona provides methods to checkout branches in Git repositories. The following snippet checks out the branch called `new-feature`.
...
Daytona provides methods to stage changes in Git repositories. You can stage specific files, all changes, and commit with a message. The following snippet stages the file `file.txt` and the directory `src`.
